...
namespace :deploy do
  ...
  task :migrate, :roles => :db, :only => { :primary => true } do
    ...
    run "cd #{directory}; #{rake} db:migrate  RAILS_ENV=#{rails_env}"
  end
...
namespace :bundle do
  task :install do
    ...
    run "cd #{gemfile_path}/ && #{bundle_cmd} install #{args.join(' ')}"
  end
end
